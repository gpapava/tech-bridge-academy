...
import { Building2, Network, BookOpen, ClipboardList, ArrowRight, Lock } from "lucide-react";
...
export default async function SMEsPage() {
  ...
  const services = [
    ...
    {
      title: "HR Coaching for Hosting Learners",
      icon: BookOpen,
    ...
    {
      title: "Skills Needs Profile",
      icon: ClipboardList,
      desc: "Complete our periodic skills needs surveys and receive a personalised report with your company's skills gap analysis, industry benchmarks, and curriculum co-design input.",
      href: "/smes/skills-needs",
      restricted: !isCompany,
      badge: "Companies Only",
    },
  ];
          ...
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-16">
            <div className="max-w-3xl">
              ...
              <p className="text-amber-100 text-lg leading-relaxed">
                A dedicated space for manufacturing companies, SMEs, and continuing education providers to network, develop HR capacities, and profile their skills needs.
              </p>
              {!isAuthenticated && (
                <div className="mt-8 flex gap-3">
                  <Link href="/auth/register?role=COMPANY" className="btn-accent">Register as Company</Link>
                  <Link href="/auth/login" className="text-amber-100 hover:text-white underline self-center text-sm">Already registered? Sign in</Link>
                </div>
              )}
            </div>
          </div>
            ...
            {services.map(({ title, icon: Icon, desc, href, restricted, badge }) => (
              <div key={title} className={`card p-6 ${restricted ? "opacity-90" : ""}`}>
                <div className="flex items-center justify-between mb-4">
                  ...
                  <span className={`text-xs font-medium rounded-full px-2.5 py-1 ${restricted ? "bg-slate-100 text-slate-500" : "bg-amber-100 text-amber-700"}`}>
                    {badge}
                  </span>
                </div>
                ...
                {restricted ? (
                  <div className="flex items-center gap-2 text-sm text-slate-400">
                    ...
                    <Link href="/auth/register?role=COMPANY" className="text-amber-600 hover:text-amber-700 font-medium">Register as Company to access</Link>
                  </div>
                ) : (
                  <Link href={href} className="flex items-center gap-1.5 text-sm font-medium text-amber-700 hover:text-amber-900">
